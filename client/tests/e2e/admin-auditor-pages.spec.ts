  ...
  test('审计员进入日志管理并导出 PRD 英文文件名 ZIP 到中文路径', async () => {
    await withDevice(async (_device, app, page) => {
      ...
      try {
        ...
        const logsTable = page.getByTestId('logs-table-shell')
        await expect(logsTable.getByText('USB插入成功').first()).toBeVisible()
        await expect(logsTable.getByText('USB移除成功')).toBeVisible()
        await expect(logsTable.getByText('授权设备, 读写, 32GB')).toBeVisible()
        await expect(logsTable.getByText('未授权设备')).toBeVisible()
        await expect(logsTable.getByText('键盘')).toBeVisible()
        await expect(logsTable.getByText('鼠标')).toBeVisible()
        await expect(logsTable.getByText('授权设备, 只读')).toBeVisible()
        await expect(logsTable.getByText('USB插入失败')).toHaveCount(0)
        await expect(logsTable.getByText('映射完成')).toHaveCount(0)
        await expect(logsTable.getByText('禁止使用')).toHaveCount(0)

        await page.getByTestId('logs-tab-malware').click()
        await expect(page.getByTestId('logs-tab-malware')).toHaveClass(/active/)
        await expect(page.getByRole('columnheader')).toHaveText(['时间', '设备名称', '序列号', '内容'])
        await expect(page.getByRole('columnheader', { name: '病毒' })).toHaveCount(0)
        await expect(page.getByText('文件: /mnt/usb/eicar.com, 病毒: EICAR-Test-File')).toBeVisible()
        await expect(page.getByText('未发现病毒文件')).toBeVisible()
        await expect(page.getByText('发现病毒并阻断')).toHaveCount(0)
  ...
  test('病毒库升级选择文件后显示居中进度弹窗且至少保持 3 秒', async () => {
    await withDevice(async (_device, app, page) => {
      ...
      try {
        ...
        await expect(progressDialog(page)).toBeVisible()
        await expectLatestMessage(page, '病毒库升级成功')
  ...
  test('授权文件上传使用居中进度和结果弹窗', async () => {
    await withDevice(async (device, app, page) => {
      ...
      try {
        ...
        await page.waitForTimeout(2_800)
        await expect(progressDialog(page)).toBeVisible()
        await expectLatestMessage(page, '授权文件上传成功')
        expect(Date.now() - startedAt).toBeGreaterThanOrEqual(3_000)
        await expect(progressDialog(page)).toBeHidden()
        await expect(licenseButton).not.toHaveClass(/is-loading/)
        await expect(licenseButton).not.toBeFocused()
