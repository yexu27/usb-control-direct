...
数据库在 DEB 安装的初始化或迁移阶段设置 WAL 模式。主服务连接已存在的数据库时只设置 5 秒锁等待超时，不修改持久化 PRAGMA，也不执行 checkpoint。

```sql
PRAGMA journal_mode = WAL;  -- 安装期设置，支持读写并发
PRAGMA busy_timeout = 5000; -- 安装程序和主服务连接均使用 5 秒锁等待超时
```
...
存储 U 盘、键盘、鼠标的物理接入和移除事件。审计记录在 udev 识别到设备、完成白名单检查后写入，不等待后续扫描和映射流程。一进一出，insert_success 与 device_remove 严格配对。

| 字段 | 类型 | 约束 | 说明 |
|---|---|---|---|
| id | INTEGER | PRIMARY KEY AUTOINCREMENT | 自增主键 |
| event_time | INTEGER | NOT NULL | 事件时间（Unix 时间戳） |
| device_type | TEXT | | 设备类型（storage / keyboard / mouse） |
| interface_type | TEXT | | 接口类型（mass_storage / hid_keyboard / hid_mouse） |
...
管理端根据 event_type 展示事件类型，设备信息（设备分类、权限、容量）通过 device_type、permission、capacity_bytes、detail 等字段组合在内容列展示。

索引：`event_time`（范围查询），`device_sn`（设备筛选），`event_type`（类型筛选），`(event_time, event_type)`（时间+类型组合查询）
...
| log_type | TEXT | NOT NULL | 操作日志分类（见下方枚举），用于结构化记录、导出、问题排查和页面“操作日志类型”列展示；不作为页面筛选条件 |
...
**装置端初始化（首次 DEB 安装执行）：**
1. `usb-control-db-migrate` 创建数据库并启用 WAL 模式
2. 创建 T01-T11 所有表
...
主服务只打开并校验已初始化数据库，不在启动时创建数据库、执行初始化 SQL、执行迁移或更新系统版本和病毒库版本。DEB 安装收尾在服务健康检查成功后，原子提交安装版本、ClamAV 实际病毒库版本和病毒库时间；病毒库包版本只由病毒库升级业务更新。
...
装置端使用 `PRAGMA user_version` 记录 Schema 版本。直接安装和在线升级均在主服务启动前调用 `usb-control-db-migrate`。

```sql
PRAGMA user_version = 1; -- 当前 schema 版本
```

数据库不存在或为空时，在一个事务内执行 `0001_init.sql`、`0001_default_data.sql` 并设置 `user_version=1`，生成 T01-T11 共 11 张业务表。

已有数据库按 `PRAGMA user_version` 依次执行目标 DEB 中尚未应用的连续迁移 SQL。每个迁移及对应的 `user_version` 更新在同一事务中提交，账号、授权、策略、系统配置和业务日志继续保留。
...
SQLite DELETE 释放的页由后续写入复用，数据库文件不保证在删除后立即缩小。装置端不在主服务启动或业务删除路径中执行 `VACUUM`、`auto_vacuum` 或 WAL 截断。
