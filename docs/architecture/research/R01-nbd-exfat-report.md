...
| 病毒文件 | 目录项可见，文件名添加 `[病毒禁止访问]` 前缀，虚拟 exFAT 目录项 DataLength 为 0；读、写、修改、删除、重命名、执行、复制等操作均禁止，目录项修改不落到真实 U 盘 |
| 可执行程序 / 黑名单 / 自动读取阻断 | 目录项可见，读取命中数据时返回 I/O error |
| 白名单只读权限 | 由 f_mass_storage write-protect（`ro=1`）实现整盘写保护 |
| 白名单读写权限 | 未命中病毒、可执行程序、黑名单和自动读取阻断的普通文件允许新增、修改、删除、重命名，变更同步写回真实 U 盘 |
...
超过 4GiB 单文件遵循同一策略判断链路，非命中策略时正常读取；命中病毒阻断时按虚拟 size=0 与全操作禁止处理，命中可执行程序、黑名单或自动读取阻断时返回 I/O error。

不引入 FAT32 输出视图、超过 4GiB 拒绝展示等替代方案。
