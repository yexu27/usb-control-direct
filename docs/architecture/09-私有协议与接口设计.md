...
message RspUploadLicense {
    ...
    string error_code = 3;      // 失败时的错误码（见统一错误码表）
...
message RspQueryLogs {
    ...
    int32 page = 6;
    int32 page_size = 7;
    string error_code = 8;      // 失败时的错误码（见统一错误码表）
...
message RspExportLogs {
    ...
    string error_code = 4;      // 失败时的错误码（见统一错误码表）
...
装置端维护每个 session token 的最后业务请求时间。超过 5 分钟无业务请求时，装置端标记该会话过期。管理端下次发送业务请求时，装置端返回 `UNAUTHENTICATED`，管理端清空进程内会话状态并跳转登录页。

- 心跳（CMD_HEARTBEAT）为连接层保活消息，不更新会话最后业务请求时间，不重置超时计时器。
- 所有携带 session_token 的业务请求更新会话最后业务请求时间。
- 会话过期后，管理端调用 CMD_LOGOUT 返回 `UNAUTHENTICATED`，管理端按正常登出处理。
