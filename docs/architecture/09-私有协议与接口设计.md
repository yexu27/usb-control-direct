...
- `add_method` 取值为 `device` 或 `management`。`device` 表示装置端添加，待添加设备必须来自 `CMD_GET_CONNECTED_DEVICES` 返回的当前连接装置设备列表，且 `device_type=storage`、`interface_type=mass_storage`、`admission_status=addable`。`management` 表示管理端添加，设备信息来自管理主机本机 USB 大容量存储设备枚举结果。
