...
message CmdQueryLogs {
    ...
    string event_type = 6;      // USB 审计事件类型筛选（insert_success/insert_failed/device_remove），空字符串表示全部
...
message UsbAuditLogEntry {
    ...
    string event_type = 7;      // insert_success / insert_failed / device_remove
    string permission = 8;
    int64 capacity_bytes = 9;
    string file_path = 10;
    string matched_policy = 11;
    string result = 12;
...
message CmdExportLogs {
    ...
    int64 end_time = 4;         // Unix 秒，包含；0 表示不限
    string keyword = 5;
    string event_type = 6;      // insert_success / insert_failed / device_remove，空字符串表示全部
}
```
