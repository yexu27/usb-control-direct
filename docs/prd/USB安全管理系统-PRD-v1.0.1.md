...
| 日志类型     | 内容                                 |
| -------- | ---------------------------------- |
| USB 审计日志 | U 盘、键盘、鼠标、未知设备的插入成功、插入失败、拔出      |
...
| 事件类型 | USB 插入成功 / USB 插入失败 / USB 拔出 |
| 内容   | 设备分类、权限、磁盘容量、处理结果等组合展示，具体格式见下方示例                            |


**USB 审计日志内容字段展示示例：**

设备分类在内容字段中统一使用 `授权设备` 和 `未授权设备`。每个设备按自身设备类型各自记录一条日志。每次物理插入/拔出事件只记录一条审计记录，细节信息（映射结果、验证结果、失败原因等）通过内容字段表达。
...
| 事件类型     | 设备分类  | 内容示例                                    |
| -------- | ----- | --------------------------------------- |
| USB 插入成功 | 授权设备  | `授权设备, 读写, 32GB, 映射完成`                  |
| USB 插入失败 | 授权设备  | `授权设备, 映射失败`                            |
| USB 插入失败 | 授权设备  | `授权设备, 挂载失败`                            |
| USB 插入失败 | 授权设备  | `授权设备, 扫描中断, U 盘拔出`                     |
| USB 插入失败 | 未授权设备 | `未授权设备, 禁止使用`                           |
| USB 拔出   | 授权设备  | `授权设备, 读写`                              |
| USB 拔出   | 未授权设备 | `未授权设备`                                 |
