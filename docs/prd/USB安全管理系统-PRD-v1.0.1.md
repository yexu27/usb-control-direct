...
> 日志记录边界：其它 USB 设备、未知接口、不支持接口默认禁止映射，仅记录装置端运行日志（tracing），不写入 USB 审计日志、恶意代码检测日志或操作日志。USB 审计日志仅记录普通 U 盘、键盘、鼠标的插入成功和移除成功；未加入白名单的普通 U 盘属于普通 U 盘准入范围，按未授权设备记录 USB 插入审计。
...
- 系统判断该 U 盘是否已加入白名单。
- 未加入白名单的普通 U 盘禁止映射，并记录 USB 插入审计，内容标识为未授权设备。
...
- 如扫描发现病毒文件，系统对命中文件添加 `[病毒禁止访问]` 前缀，并禁止受控主机侧读取命中文件，同时记录恶意代码检测日志。
- 如扫描未发现病毒文件，继续后续处理。
...
- 验证过程中键盘输入不得传递到受控主机。
- 键盘插入成功和移除成功需记录 USB 审计日志；验证失败和映射失败等过程信息记录为装置运行日志，不进入 USB 审计日志。
- 本版本不提供指示灯状态提示。
...
- 鼠标插入成功和移除成功需记录 USB 审计日志；映射失败等过程信息记录为装置运行日志，不进入 USB 审计日志。
...
- USB 审计日志仅记录 U 盘、键盘、鼠标的插入成功和移除成功；映射失败、禁止原因等过程信息记录为装置运行日志或对应业务日志。
- 管理端流程需符合三权分立权限边界。
...
- 未加入白名单的普通 U 盘插入记录为 USB 审计日志的未授权设备；白名单 U 盘插入记录为授权设备；映射失败等过程信息记录为装置运行日志。
...
- 验证失败或未验证时不得映射。
- 验证过程中键盘输入不得传递到受控主机。
- 键盘插入成功和移除成功记录 USB 审计日志；验证失败和映射失败等过程信息记录为装置运行日志。
- 本版本不提供指示灯状态提示。
...
- 鼠标插入后，系统自动检测并完成映射。
- 鼠标插入成功和移除成功记录 USB 审计日志；映射失败等过程信息记录为装置运行日志。
...
- 鼠标插入后自动映射。
- 安全 U 盘专属能力不进入本版本。
- USB 审计日志仅记录 U 盘、键盘、鼠标的插入成功和移除成功；禁止、映射失败等过程信息不进入 USB 审计日志。
...
- 未在白名单中的普通移动存储设备插入装置后，禁止使用并记录 USB 插入审计，内容标识为未授权设备。
...
- 未认证 U 盘插入需记录 USB 审计日志，内容标识为未授权设备。
- 白名单设备插入和拔出需记录 USB 审计日志，内容标识为授权设备及权限、容量信息；映射失败等过程信息记录为装置运行日志。
...
- 上述伪装后缀规则不影响病毒扫描结果；病毒命中文件仍按恶意代码检测规则禁止读取。
...
病毒文件始终禁止读取。U 盘只读 / 读写权限只作用于允许访问的文件，不覆盖病毒文件阻断和文件访问控制规则。
...
- 用户访问被文件访问控制策略阻断的文件时，由装置端记录运行日志（tracing），不入 USB 审计数据库。
...
- 命中的恶意代码文件禁止读取。除目录枚举可展示带前缀的文件名外，受控主机侧对该文件发起的读取操作均应被拒绝。
...
| 发现病毒文件      | 标记命中文件，禁止读取命中文件，记录恶意代码检测日志，U 盘其它文件继续可用 |
...
无真实病毒样本时，需通过测试样本或模拟命中完成扫描、命中、标记、读取阻断和日志记录链路验收。
...
| 扫描过程中 U 盘拔出     | 中止扫描，不写入本次扫描结果，不映射该次接入，记录扫描中断日志 |
| 病毒文件无法添加前缀      | 仍禁止读取该文件，并记录处理失败原因            |
| 病毒文件访问失败提示不可展示  | 至少保证对该文件的读取操作被拒绝，并记录日志       |
...
- 病毒扫描完成前，U 盘不映射为可用状态。
- 命中的病毒文件添加 `[病毒禁止访问]` 前缀；除目录枚举展示文件名外，对该文件的读取操作均被拒绝。
- 病毒命中后，未命中的其它文件继续可用。
- 病毒扫描结果记录恶意代码检测日志。
- 系统管理员可上传 `.zip` 病毒库升级包。
- 病毒库升级成功后版本和更新时间更新。
...
| 事件类型 | USB 插入成功 / USB 移除成功 |
| 内容   | 设备分类、权限、磁盘容量等组合展示，具体格式见下方示例                            |


**USB 审计日志内容字段展示示例：**

设备分类在内容字段中统一使用 `授权设备` 和 `未授权设备`。仅记录 U 盘、键盘、鼠标三类设备，不支持的设备类型不入审计数据库。每个设备按自身设备类型各自记录一条日志。每次物理插入/移除事件只记录一条审计记录。审计记录在 udev 识别到设备的时刻写入，不等待后续映射流程完成。
...
| 设备名称 | 系统识别到的设备名称                                                |
| 序列号  | 普通 U 盘序列号                                                 |
| 内容   | 按扫描结果组装：未发现病毒文件；文件: `{file_path}`, 病毒: `{virus_name}`；扫描失败: `{fail_reason}`；扫描中断: 设备拔出 |
...
| 内容     | 具体操作事件、对象、变更详情、结果和失败原因，由操作日志结构化字段组合展示                  |
...
- 普通 U 盘插入成功，区分授权设备和未授权设备
- 普通 U 盘移除成功
- 键盘插入成功和移除成功
- 鼠标插入成功和移除成功
- USB 审计事件类型仅包含 `insert_success` 和 `device_remove`

> 映射失败、挂载失败、键盘验证失败、鼠标映射失败、未知设备拒绝、文件访问控制阻断等过程信息记录为装置运行日志或对应业务日志，不进入 USB 审计日志。
...
- U 盘、键盘、鼠标的插入成功和移除成功需记录 USB 审计日志；映射、禁止、阻断、扫描中断等过程信息记录为运行日志、恶意代码检测日志或操作日志。
...
| 病毒文件处理   | 病毒文件添加 `[病毒禁止访问]` 前缀；除目录枚举展示文件名外，对该文件的读取操作均被拒绝，其它文件继续可用 |
...
| USB 审计日志          | 记录 U 盘、键盘、鼠标插入成功和移除成功的日志        | 不记录未知设备、映射失败、禁止或阻断过程 |
...
| 未认证  | 普通 U 盘未加入白名单  | 禁止映射，记录 USB 插入审计为未授权设备 |
| 已认证  | 普通 U 盘已加入白名单  | 进入扫描流程            |
| 扫描中  | 病毒扫描未完成       | 不映射为可用状态          |
| 扫描完成 | 病毒扫描完成        | 进入文件访问控制和权限映射     |
| 扫描失败 | 病毒扫描未完成       | 不完成可用映射；记录恶意代码检测日志并提示失败 |
| 映射成功 | U 盘成功映射至受控主机  | 用户按权限使用           |
| 映射失败 | U 盘未成功映射至受控主机 | 记录运行日志并提示失败       |
| 已拔出  | U 盘从装置移除      | 记录拔出事件            |
