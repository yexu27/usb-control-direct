# USB 管控系统直连版产品需求文档 v1.0.1

## 1. 产品概述

### 1.1 产品定位

USB 管控系统直连版是一款面向单台受控主机的 USB 外设安全管控产品。通过硬件装置接管 USB 接口，在 USB 外设与受控主机之间提供接入管控、介质映射、病毒检测、文件访问控制、日志审计、授权管理和系统维护能力，降低普通 U 盘、键盘、鼠标等外设接入带来的安全风险。
...
- 用户首次登录或密码被重置后，可提示修改密码，本版本不强制修改。
...
- 命中的恶意代码文件添加 `[病毒禁止访问]` 前缀。该前缀修改的是 U 盘上的实际文件名，用户在受控主机文件管理器中可以看到带前缀的文件名。U 盘拔出后在其它设备上查看，文件名同样带有该前缀。
...
| 修改设备描述时装置连接 USB 设备 | 禁止修改，提示请先移除 USB 设备         |
...
| 自定义设备描述  | 支持修改自定义设备描述，满足格式约束，重启后生效                                  |
