...
impl Storage {
    ...
    pub fn usb_audit_insert(&self, item: &UsbAuditLogInsert) -> Result<i64, StorageError> {
        ...
        self.pool().with_transaction(|tx| {
            tx.execute(
                "INSERT INTO usb_audit_log (event_time, device_type, interface_type, interface_class, interface_subclass, interface_protocol, device_name, device_sn, vid, pid, event_type, permission, capacity_bytes, detail) \
    ...
    pub fn usb_audit_query_by_time(&self, from: i64, to: i64) -> Result<Vec<UsbAuditLog>, StorageError> {
        self.pool().with_read(|conn| {
            let mut stmt = conn.prepare(
                "SELECT id, event_time, device_type, interface_type, interface_class, interface_subclass, interface_protocol, device_name, device_sn, vid, pid, event_type, permission, capacity_bytes, detail \
        ...
        self.pool().with_read(|conn| {
            ...
            let query_sql = format!(
                "SELECT id, event_time, device_type, interface_type, interface_class, \
                 ...
                 event_type, permission, capacity_bytes, detail \
